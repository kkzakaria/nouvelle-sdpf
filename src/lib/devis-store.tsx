...
export function DevisProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { setDevis(load()) }, [])
  useEffect(() => {
    if (typeof localStorage !== 'undefined') localStorage.setItem(KEY, JSON.stringify(devis))
  }, [devis])

  const toggle = useCallback((id: string) => {
    setDevis((d) => { const n = { ...d }; if (n[id]) delete n[id]; else n[id] = 1; return n })
  }, [])
  const setQty = useCallback((id: string, qty: number) => {
    setDevis((d) => { const n = { ...d }; if (qty <= 0) delete n[id]; else n[id] = Math.min(qty, 999); return n })
  }, [])
